...
app.get('/', (req, res) => {
  ...
  db.all('SELECT username, server, IFNULL(SUM(score), 0) AS score, SUM(1) AS total FROM Match LEFT JOIN User USING(id) GROUP BY id, server ORDER BY score DESC', (err, rows) => {
    if(err) {
