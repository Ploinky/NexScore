...
app.get('/', (req, res) => {
  ...
  db.all('SELECT username, server, IFNULL(SUM(score), 0) AS score, SUM(1) AS total, CAST(SUM(score) AS DECIMAL) / SUM(1) * 100 AS pct FROM Match LEFT JOIN User USING(id) GROUP BY id, server ORDER BY score DESC', (err, rows) => {
    if(err) {
